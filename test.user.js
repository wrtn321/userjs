...
(function() {
    ...
    async function handleCapture() {
        ...
        try {
            ...
            // 너비는 이전 버전(2.1)과 동일하게 좌우 여백을 포함하여 계산합니다.
            // 이렇게 해야 텍스트가 잘리지 않습니다.
            ...
            // 위쪽 여백은 0으로 설정하고, 좌우/아래 여백은 유지합니다.
            ...
            selectedMessages.reverse().forEach(msg => {
                ...
                // ========================= ★★★ 수정된 부분 ★★★ =========================
                //
                // 각 메시지 창(clone) 안에서 실제 텍스트 내용을 담고 있는 부분을 찾습니다.
                // 보통 'prose'라는 클래스를 가진 div가 이 역할을 합니다.
                const textBlock = clone.querySelector('.prose');

                // 만약 텍스트 영역을 찾았다면,
                if (textBlock) {
                    // CSS의 transform 속성을 이용해 Y축(세로)으로 -10px 만큼 이동시킵니다.
                    // 이렇게 하면 메시지 풍선은 그대로 있고, 텍스트 블록만 위로 10px 올라갑니다.
