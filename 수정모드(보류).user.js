...
(function() {
    ...
    const VIEWER_JS = function() {
        ...
        // --- 미니 보안 요원 ---
        function safeUrl(url) { try { const parsedUrl = new URL(url, window.location.href); if (['http:', 'https:'].includes(parsedUrl.protocol)) { return url; } } catch (e) { return '#invalid-url'; } return '#unsafe-protocol'; }

        // --- 마크다운 파서 ---
        function parseInlineMarkdown(text) { let htmlLine = text; htmlLine = htmlLine.replace(/!\[(.*?)\]\((.*?)\)/g, (match, alt, src) => `<img src="${escapeHtml(safeUrl(src))}" alt="${escapeHtml(alt)}" style="max-width: 100%; border-radius: 8px;">`); htmlLine = htmlLine.replace(/(\*\*\*|___|\*\*|__)(.+?)\1/g, '<strong>$2</strong>'); htmlLine = htmlLine.replace(/(\*|_)(.+?)\1/g, '<em style="font-style:normal;color:#9a9a9a;">$2</em>'); htmlLine = htmlLine.replace(/~~(.+?)~~/g, '<del>$1</del>'); htmlLine = htmlLine.replace(/\^\^(.+?)\^\^/g, '<mark>$1</mark>'); htmlLine = htmlLine.replace(/`(.+?)`/g, (match, code) => `<code style="font-weight: bold; background-color: rgba(0,0,0,0.05); padding: 2px 4px; border-radius: 4px;">${escapeHtml(code)}</code>`); return htmlLine; }
        function parseMarkdown(text) { if (!text) return ''; const lines = text.split('\n'); const htmlBlocks = []; for (let i = 0; i < lines.length; i++) { let line = lines[i]; if (line.trim().startsWith('```')) { const lang = line.trim().substring(3).trim(); const codeLines = []; i++; while (i < lines.length && !lines[i].trim().startsWith('```')) { codeLines.push(lines[i]); i++; } const langHeader = lang ? `<div style="background-color: #4a4a4a; color: #e0e0e0; padding: 5px 10px; border-top-left-radius: 6px; border-top-right-radius: 6px;">${escapeHtml(lang)}</div>` : ''; htmlBlocks.push(`<div style="background-color: #2d2d2d; border-radius: 6px; margin: 1em 0;">${langHeader}<pre style="margin: 0;"><code style="color:#f1f1f1; padding: 10px; display: block; white-space: pre-wrap; word-wrap: break-word;">${escapeHtml(codeLines.join('\n'))}</code></pre></div>`); continue; } if (line.includes('|') && i + 1 < lines.length && lines[i+1].includes('-')) { if (lines[i+1].trim().replace(/\|/g, '').replace(/-/g, '').replace(/:/g, '').replace(/\s/g, '') === '') { const headers = line.split('|').slice(1, -1).map(h => `<th>${parseInlineMarkdown(h.trim())}</th>`).join(''); const bodyLines = []; i += 2; while (i < lines.length && lines[i].includes('|')) { bodyLines.push(lines[i]); i++; } i--; const rows = bodyLines.map(rowLine => { const cells = rowLine.split('|').slice(1, -1).map(c => `<td>${parseInlineMarkdown(c.trim())}</td>`).join(''); return `<tr>${cells}</tr>`; }).join(''); htmlBlocks.push(`<table><thead><tr>${headers}</tr></thead><tbody>${rows}</tbody></table>`); continue; } } const hMatch = line.match(/^(#+) (.*)$/); if (hMatch) { const level = hMatch[1].length; htmlBlocks.push(`<h${level} style="font-weight: bold; font-size: ${2.0 - level * 0.2}em; margin: 0;">${parseInlineMarkdown(hMatch[2])}</h${level}>`); continue; } if (/^(---|___|\*\*\*)$/.test(line.trim())) { htmlBlocks.push('<hr>'); continue; } if (line.trim() !== '') { htmlBlocks.push(`<p>${parseInlineMarkdown(escapeHtml(line))}</p>`); } } return htmlBlocks.join(''); }
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // [기능 수정 2] 새로 추가한 이름 입력창(input)을 변수로 가져옵니다.
            ...
            function renderAll() {
                ...
                // [기능 수정 3] 뷰어 로딩 시, 이름 입력창에도 현재 프로필 이름을 채워줍니다.
                personaNameInput.value = window.initialChatData.userPersona.name || '프로필';
            ...
            function createMessageBubble(message, index) {
                ...
                const enterEditMode = () => { if (activeEditingIndex !== null) { showToast('다른 항목 수정을 먼저 완료하세요.'); return; } activeEditingIndex = index; bubble.classList.add('editing'); viewContent.style.display = 'none'; editContainer.style.display = 'block'; autoResizeTextarea(editTextarea); editTextarea.focus({ preventScroll: true }); };
                const exitEditMode = (save) => { if (save) { const newContent = editTextarea.value; viewContent.innerHTML = parseMarkdown(newContent); bubble.dataset.content = newContent; } else { editTextarea.value = bubble.dataset.content; } bubble.classList.remove('editing'); viewContent.style.display = 'block'; editContainer.style.display = 'none'; activeEditingIndex = null; };
            ...
            function getCurrentDataFromDOM() {
                return {
                    ...
                    userPersona: {
                        ...
                        information: personaInfoEl.textContent // 설명 부분도 저장된 값(textContent)을 기준으로 하도록 수정하여 일관성 확보
                    },
                    userNote: usernoteInfoEl.textContent, // 유저노트도 저장된 값(textContent)을 기준으로 하도록 수정
            ...
            function download(content, filename, contentType) { const blob = new Blob([content], { type: contentType }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = filename; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href); }
            ...
            function addEventListeners() {
                ...
                viewerTitle.addEventListener('click', () => { if (document.querySelector('.title-edit-input')) return; const input = document.createElement('input'); input.type = 'text'; input.className = 'title-edit-input'; input.value = viewerTitle.textContent; viewerTitle.style.display = 'none'; viewerTitle.parentNode.insertBefore(input, viewerTitle); input.focus(); const saveTitle = () => { const newTitle = input.value.trim() || '제목 없음'; viewerTitle.textContent = newTitle; document.title = newTitle; viewerTitle.style.display = 'block'; if (input.parentNode) input.parentNode.removeChild(input); }; input.addEventListener('blur', saveTitle); input.addEventListener('keydown', e => { if (e.key === 'Enter') input.blur(); }); });
                ...
                const setupEditToggle = type => {
                    ...
                    // [기능 수정 4] 'persona' 수정일 경우에만 이름 입력창을 다루도록 로직 추가
                    const nameInput = (type === 'persona') ? document.getElementById('persona-name-input') : null;
                    const nameEl = (type === 'persona') ? document.getElementById('persona-name') : null;
                    ...
                    document.getElementById(`cancel-${type}-btn`).addEventListener('click', () => {
                        // [기능 수정 5] '취소' 버튼 클릭 시 이름과 설명 모두 원래 데이터로 되돌립니다.
                        if (type === 'persona') {
                            nameInput.value = window.initialChatData.userPersona.name;
                            textarea.value = window.initialChatData.userPersona.information;
                        } else { // usernote
                            textarea.value = window.initialChatData.userNote;
                        }
                    ...
                    document.getElementById(`save-${type}-btn`).addEventListener('click', () => {
                        // [기능 수정 6] '저장' 버튼 클릭 시 이름과 설명을 모두 저장하고 화면에 반영합니다.
                        if (type === 'persona') {
                            const newName = nameInput.value.trim() || '프로필';
                            const newInfo = textarea.value;
                            nameEl.textContent = newName;
                            infoEl.textContent = newInfo;
                            window.initialChatData.userPersona.name = newName;
                            window.initialChatData.userPersona.information = newInfo;
                        } else { // usernote
                            const newValue = textarea.value;
                            infoEl.textContent = newValue;
                            window.initialChatData.userNote = newValue;
                        }
                        document.getElementById(`${type}-view-mode`).hidden = false;
                        document.getElementById(`${type}-edit-mode`).hidden = true;
                ...
                document.getElementById('download-html-btn').addEventListener('click', e => { e.preventDefault(); const currentData = getCurrentDataFromDOM(); const serializedData = JSON.stringify(currentData, null, 2).replace(/</g, '\\u003C').replace(/>/g, '\\u003E'); const regeneratedHtml = `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no, maximum-scale=1.0, minimum-scale=1.0"><title>${escapeHtml(currentData.title)}</title><style>${window.CSS_TEMPLATE}</style></head><body class="sticky-footer-layout">${window.HTML_TEMPLATE}<script>window.initialChatData = ${serializedData}; window.HTML_TEMPLATE = ${JSON.stringify(window.HTML_TEMPLATE)}; window.CSS_TEMPLATE = ${JSON.stringify(window.CSS_TEMPLATE)}; window.VIEWER_JS_SOURCE = ${JSON.stringify(window.VIEWER_JS_SOURCE)};<\/script><script>(new Function(window.VIEWER_JS_SOURCE))();<\/script></body></html>`; download(regeneratedHtml, `${currentData.title.replace(/[\\/:*?"<>|]/g,"")}.html`, "text/html;charset=utf-8"); });
            ...
            const showToast = message => { const toast = document.getElementById('toast-notification'); if (!toast) return; toast.querySelector('.toast-message').textContent = message; clearTimeout(toastTimer); toast.classList.add('show'); toastTimer = setTimeout(() => toast.classList.remove('show'), 2500); };
            renderAll();
            addEventListeners();
        });
    };
    ...
    // ===================================================================================
    // PART 2: WRTN.AI 사이트 로직 (이 부분은 변경사항 없습니다)
    // ===================================================================================

    function generateFullHtmlPage(chatData) { function escapeHtml(unsafe) { if (typeof unsafe !== 'string') return ''; return unsafe.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;").replace(/'/g, "&#039;");} const serializedData = JSON.stringify(chatData, null, 2).replace(/</g, '\\u003C').replace(/>/g, '\\u003E'); const viewerJsString = `(${VIEWER_JS.toString()})();`; return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, user-scalable=no, maximum-scale=1.0, minimum-scale=1.0"><title>${escapeHtml(chatData.title)}</title><style>${VIEWER_CSS}</style></head><body class="sticky-footer-layout">${HTML_STRUCTURE}<script>window.initialChatData = ${serializedData}; window.HTML_TEMPLATE = ${JSON.stringify(HTML_STRUCTURE)}; window.CSS_TEMPLATE = ${JSON.stringify(VIEWER_CSS)}; window.VIEWER_JS_SOURCE = ${JSON.stringify(viewerJsString)};<\/script><script>(new Function(window.VIEWER_JS_SOURCE))();<\/script></body></html>`; }
    ...
    async function fetchAllChatData() { const token = getCookie('access_token'); const { chatroomId } = getUrlInfo(); if (!token || !chatroomId) throw new Error('토큰 또는 채팅방 ID를 찾을 수 없습니다.'); const API_BASE = "https://contents-api.wrtn.ai"; const chatroomPromise = apiRequest(`${API_BASE}/character-chat/api/v2/chat-room/${chatroomId}`, token); const messagesPromise = apiRequest(`${API_BASE}/character-chat/api/v2/chat-room/${chatroomId}/messages?limit=2000`, token); const personaListPromise = apiRequest(`${API_BASE}/character/character-profiles`, token) .then(p => p?.wrtnUid ? apiRequest(`${API_BASE}/character/character-profiles/${p.wrtnUid}`, token) : null) .then(pd => pd?._id ? apiRequest(`${API_BASE}/character/character-profiles/${pd._id}/character-chat-profiles`, token) : { characterChatProfiles: [] }) .then(list => list.characterChatProfiles || []); const [chatroomData, messagesData, personaList] = await Promise.all([chatroomPromise, messagesPromise, personaListPromise]); const messages = (messagesData?.list || []).reverse().map(m => ({ role: m.role, content: m.content })); const currentPersonaId = chatroomData?.chatProfile?._id; let currentPersona = currentPersonaId ? personaList.find(p => p._id === currentPersonaId) : null; if (!currentPersona && personaList.length > 0) { currentPersona = personaList.find(p => p.isRepresentative); } return { title: chatroomData?.title || 'Unknown Chat', userPersona: { name: currentPersona?.name || '적용된 프로필 없음', information: currentPersona?.information || '' }, userNote: chatroomData?.character?.userNote?.content || '', messages: messages }; }
    async function createMenuButton() { try { const menuContainer = await waitForElement('.css-uxwch2'); const buttonId = 'html-viewer-saver-final-secure'; if (document.getElementById(buttonId)) return; const button = document.createElement('div'); button.id = buttonId; button.className = 'css-1dib65l'; button.style.cssText = "display: flex; cursor: pointer; padding: 10px;"; button.innerHTML = `<p class="css-1xke5yy"><span style="padding-right: 6px;">📄</span>HTML 뷰어 저장</p>`; button.addEventListener('click', async () => { const p = button.querySelector('p'); const originalText = p.innerHTML; try { p.textContent = '생성 중...'; button.style.pointerEvents = 'none'; const chatData = await fetchAllChatData(); const finalHtml = generateFullHtmlPage(chatData); const fileName = `${chatData.title.replace(/[\\/:*?"<>|]/g, '')}.html`; downloadFile(finalHtml, fileName, 'text/html;charset=utf-8'); } catch (error) { console.error('HTML 생성 실패:', error); alert(`오류가 발생했습니다: ${error.message}`); } finally { p.innerHTML = originalText; button.style.pointerEvents = 'auto'; } }); menuContainer.appendChild(button); } catch (e) { console.error('버튼 생성 실패:', e); } }
