// ==UserScript==
// @name         Wrtn 채팅 뷰어(HTML) 생성기 v3.7
// @namespace    http://tampermonkey.net/
// @version      3.7
// @description  하이브리드 마크다운 및 모든 UI 요청사항을 반영한 최종 결정판입니다.
// @author       Your name
    ...
    const VIEWER_JS = function() {
        ...
        function parseMarkdown(text) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (quoteMatch) {
                    let quoteDepth = quoteMatch[1].length;
                    let quoteContent = parseInlineMarkdown(quoteMatch[2]);
                    let j = i + 1;
                    for (; j < lines.length; j++) {
                        const nextLineMatch = lines[j].match(/^(>+)\s?(.*)/);
                        if (nextLineMatch && nextLineMatch[1].length === quoteDepth) {
                            quoteContent += '<br>' + parseInlineMarkdown(nextLineMatch[2]);
                        } else {
                            break;
                        }
                    }
                    i = j - 1;
                    for (let k = 0; k < quoteDepth; k++) {
                        quoteContent = `<blockquote style="border-left: 3px solid #ccc; padding-left: 10px; margin: 0 0 0 ${k > 0 ? '10px' : '0'};">${quoteContent}</blockquote>`;
                    }
                    htmlBlocks.push(quoteContent);
                    continue;
                }
                // Gemini 버전 코드블럭
                ...
                if (line.trim() !== '') { htmlBlocks.push(`<p>${parseInlineMarkdown(line)}</p>`); }
            }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            let activeEditingIndex = null; let toastTimer;
            ...
            function renderAll() {
                ...
                window.initialChatData.messages.forEach((msg, index) => chatLogContainer.appendChild(createMessageBubble(msg, index)));
            ...
            function addEventListeners() {
                ...
                const setupEditToggle = type => {
                    ...
                    document.getElementById(`save-${type}-btn`).addEventListener('click', () => {
                        ...
                        if (type === 'persona') window.initialChatData.userPersona.information = newValue; else window.initialChatData.userNote = newValue;
                        document.getElementById(`${type}-view-mode`).hidden = false;
                        document.getElementById(`${type}-edit-mode`).hidden = true;
...
<\/script>
    ...
    async function createMenuButton() {
        try {
            ...
            const buttonId = 'html-viewer-saver-v3.6';
            if (document.getElementById(buttonId)) return;
            const button = document.createElement('div');
            button.id = buttonId;
            button.className = 'css-1dib65l';
            button.style.cssText = "display: flex; cursor: pointer; padding: 10px;";
            button.innerHTML = `<p class="css-1xke5yy"><span style="padding-right: 6px;">📄</span>HTML 뷰어 저장 v3.6</p>`;
