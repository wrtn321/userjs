...
// @version      4.0
// @description  하이브리드 방식으로 오프라인과 안정성을 모두 지원하며, JSON 취약점을 해결한 최종 버전입니다
// @author       뤼붕이
...
(function() {
    ...
    // ===================================================================================
    // PART 1: 뷰어 템플릿 및 스타일 (사용자 수정본 유지)
    // ===================================================================================
    ...
    function viewerJsLogic() {
        ...
        function parseInlineMarkdown(text) {
            let htmlLine = text;
            htmlLine = htmlLine.replace(/</g, '&lt;').replace(/>/g, '&gt;');
            ...
            htmlLine = htmlLine.replace(/(\*|_)(.+?)\1/g, '<em>$2</em>');
        ...
        function parseMarkdown(text) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (line.trim().startsWith('>')) {
                    ...
                    i--;
                    htmlBlocks.push(`<blockquote>${parseInlineMarkdown(quoteLines.join('<br>'))}</blockquote>`);
                    continue;
                }
                if (line.trim().startsWith('```')) {
                    ...
                    htmlBlocks.push(`<div class="code-block">${langHeader}<pre><code>${codeContent}</code></pre></div>`);
                    continue;
                }
                ...
                if (hMatch) {
                    ...
                    htmlBlocks.push(`<h${level}>${parseInlineMarkdown(hMatch[2])}</h${level}>`);
                    continue;
                }
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            function createMessageBubble(message, index) {
                ...
                renderContent(viewContent, message.content);
                ...
                const exitEditMode = (save) => {
                    if (save) {
                        ...
                        renderContent(viewContent, newContent);
            ...
            function addEventListeners() {
                ...
                document.getElementById('download-html-btn').addEventListener('click', (e) => {
                    ...
                    const regeneratedHtml = generateCompleteHtml(currentData, {
                        HTML_TEMPLATE: window.HTML_TEMPLATE,
                        CSS_TEMPLATE: window.CSS_TEMPLATE,
                        VIEWER_JS_SOURCE_CODE: window.VIEWER_JS_SOURCE_CODE,
                        DOMPURIFY_SOURCE_CODE: window.DOMPURIFY_SOURCE_CODE
                    });
                    const blob = new Blob([regeneratedHtml], { type: 'text/html;charset=utf-8' });
                    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${currentData.title.replace(/[\\/:*?"<>|]/g,"")}.html`;
                    document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href);
                });
                document.getElementById('download-json-btn').addEventListener('click', e => { e.preventDefault(); const d = getCurrentDataFromDOM(); const blob = new Blob([JSON.stringify(d, null, 2)], { type: 'application/json;charset=utf-8' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${d.title.replace(/[\\/:*?"<>|]/g,"")}.json`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href); });
                document.getElementById('download-txt-btn').addEventListener('click', e => { e.preventDefault(); const d = getCurrentDataFromDOM(); const c = d.messages.map(m => `${m.role.toUpperCase()}:\n${m.content}`).join('\n\n'); const blob = new Blob([c], { type: 'text/plain;charset=utf-8' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${d.title.replace(/[\\/:*?"<>|]/g,"")}.txt`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(a.href); });
            }
    ...
    // ===================================================================================
    // PART 3: 메인 스크립트 로직 (사이트에서 실행)
    // ===================================================================================

    // DOMPurify 라이브러리 코드를 저장할 변수 (메모리 캐싱 용도)
    let dompurifySource = null;

    // [수정] CDN에서 DOMPurify 라이브러리를 가져오는 함수
    function fetchDomPurify() {
        // 이미 가져온 적이 있다면, 네트워크 요청 없이 캐시된 데이터를 즉시 반환
        if (dompurifySource) return Promise.resolve(dompurifySource);
            ...
            GM.xmlHttpRequest({
                ...
                onload: (response) => {
                    if (response.status === 200) {
                        dompurifySource = response.responseText; // 성공 시 코드를 캐시에 저장
                        resolve(dompurifySource);
                    } else {
                        reject(new Error(`DOMPurify 로드 실패: ${response.status}`));
                    }
                },
                onerror: (error) => reject(new Error(`DOMPurify 로드 중 네트워크 오류: ${error}`))
            });
        });
    }

    // HTML 파일 전체를 생성하는 함수
    function generateCompleteHtml(chatData, templates) {
        const purifyScriptContent = templates.DOMPURIFY_SOURCE_CODE;
        const viewerScriptContent = templates.VIEWER_JS_SOURCE_CODE;

        // [수정] JSON 데이터를 <script> 태그에 안전하게 삽입하는 로직
        const safeInitialData = JSON.stringify(chatData).replace(/</g, '\\u003C');
        const safeHtmlTemplate = JSON.stringify(templates.HTML_TEMPLATE).replace(/</g, '\\u003C');
        const safeCssTemplate = JSON.stringify(templates.CSS_TEMPLATE).replace(/</g, '\\u003C');
        const safeViewerJs = JSON.stringify(viewerScriptContent).replace(/</g, '\\u003C');
        const safeDompurify = JSON.stringify(purifyScriptContent).replace(/</g, '\\u003C');
...
<head>
    ...
    <style>${templates.CSS_TEMPLATE}</style>
    <script>${purifyScriptContent}<\/script>
</head>
<body class="sticky-footer-layout">
${templates.HTML_TEMPLATE}
<script>
    window.initialChatData = ${safeInitialData};
    window.HTML_TEMPLATE = ${safeHtmlTemplate};
    window.CSS_TEMPLATE = ${safeCssTemplate};
    window.VIEWER_JS_SOURCE_CODE = ${safeViewerJs};
    window.DOMPURIFY_SOURCE_CODE = ${safeDompurify};
<\/script>
<script>
    (${viewerScriptContent})();
<\/script>
    ...
    async function fetchAllChatData() {
        ...
        const [chatroomData, messagesData, personaList] = await Promise.all([
            apiRequest(`${API_BASE}/character-chat/api/v2/chat-room/${chatroomId}`, token),
            apiRequest(`${API_BASE}/character-chat/api/v2/chat-room/${chatroomId}/messages?limit=2000`, token),
            apiRequest(`${API_BASE}/character/character-profiles`, token)
    ...
    async function createMenuButton() {
        try {
            ...
            const buttonId = 'html-viewer-saver-hybrid';
            if (document.getElementById(buttonId)) return;
            const button = document.createElement('div');
            button.id = buttonId;
            button.className = 'css-1dib65l';
            button.style.cssText = "display: flex; cursor: pointer; padding: 10px;";
            button.innerHTML = `<p class="css-1xke5yy"><span style="padding-right: 6px;">📄</span>HTML 저장</p>`;
            button.addEventListener('click', async () => {
                const p = button.querySelector('p');
                const originalText = p.innerHTML;
                try {
                    p.textContent = '보안 라이브러리 로딩 중...';
                    button.style.pointerEvents = 'none';
                    // [수정] 버튼 클릭 시, CDN에서 검증된 라이브러리를 가져옵니다.
                    const purifySource = await fetchDomPurify();

                    p.textContent = '채팅 데이터 수집 중...';
                    const chatData = await fetchAllChatData();

                    p.textContent = 'HTML 파일 생성 중...';
                    const finalHtml = await generateCompleteHtml(chatData, {
                        HTML_TEMPLATE: HTML_STRUCTURE,
                        CSS_TEMPLATE: VIEWER_CSS,
                        VIEWER_JS_SOURCE_CODE: viewerJsLogic.toString(),
                        DOMPURIFY_SOURCE_CODE: purifySource // 가져온 라이브러리를 전달
                    });
